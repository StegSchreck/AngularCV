...
export class PdfComponent implements OnInit {
  ...
  private addGeneralDescription(doc) {
    ...
    doc.text(this.l10n.overview.subtitle_aboutme.toUpperCase(), 20, this.verticalPosition);
    this.verticalPosition += 10;
    doc.setFontSize(12);
    doc.setFontType('normal');
    const splittedDescription = doc.splitTextToSize(this.generalData.description, this.maximumHorizontalLength);
    doc.text(splittedDescription, 20, this.verticalPosition);
  }

  private addContactData(doc) {
    this.addLine(doc, this.l10n.pdf.label_city.toUpperCase(), this.contactItems.city);
    this.addLine(doc, this.l10n.pdf.label_email.toUpperCase(), this.contactItems.mail);
    this.addLine(doc, this.l10n.contact.phone.toUpperCase(), this.contactItems.phone);
  ...
  private addLanguages(doc) {
    if (this.languageItems !== undefined && this.languageItems !== [] && this.languageItems.length > 0) {
      ...
      doc.setFontSize(14);
      doc.setFontType('bold');
      doc.text(this.l10n.education.title_languages.toUpperCase(), 20, this.verticalPosition);
      this.verticalPosition += 10;
      doc.setFontSize(12);
      doc.setFontType('normal');
  ...
  private addInterests(doc) {
    if (this.interestItems !== undefined && this.interestItems !== [] && this.interestItems.length > 0) {
      doc.setFontSize(14);
      doc.setFontType('bold');
      doc.text(this.l10n.overview.subtitle_interests.toUpperCase(), 20, this.verticalPosition);
      this.verticalPosition += 10;
      doc.setFontSize(12);
      doc.setFontType('normal');
      let interests = '';
      ...
      const splittedInterests = doc.splitTextToSize(interests, this.maximumHorizontalLength);
  ...
  private addExperiencePageContent(doc) {
    this.addPageTitle(doc, this.l10n.experience.title);
  ...
  private addCvItems(doc, items: CvItem[]) {
    for (const item of items) {
      if (this.verticalPosition >= 235) {
        ...
        doc.text(this.l10n.pdf.continuing_next_page, 20, this.verticalPosition);
        this.switchPage(doc);
        doc.setTextColor(68, 68, 68);
        doc.setFontType('bolditalic');
        doc.setFontSize(10);
        doc.text(this.l10n.pdf.continued_from_last_page, 20, this.verticalPosition);
  ...
  private addCvItem(doc, item: CvItem) {
    ...
    if (item.description !== undefined && item.description !== '') {
      ...
      doc.setTextColor(0, 0, 0);
      const splittedDescription = doc.splitTextToSize(item.description, this.maximumHorizontalLength - 10);
    ...
    if (item.tags !== undefined && item.tags.length > 0) {
      ...
      const splittedTags = doc.splitTextToSize(tags, this.maximumHorizontalLength - 10);
  ...
  private addPageFooter(doc) {
    ...
    doc.setFontType('normal');
    doc.setFontSize(7);
    doc.text(this.l10n.pdf.info_generation, 20, 284);
    doc.setFontSize(7);
    doc.text(this.l10n.pdf.info_headhunter, 20, 287);
