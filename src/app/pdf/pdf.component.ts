...
export class PdfComponent implements OnInit {
  ...
  private addLanguages(doc) {
    if (this.languageItems !== undefined && this.languageItems !== [] && this.languageItems.length > 0) {
      ...
      doc.setFontSize(14);
      doc.setFontType('bold');
      doc.text('LANGUAGES', 20, this.verticalPosition);
      this.verticalPosition += 10;
      doc.setFontSize(12);
      doc.setFontType('normal');
  ...
  private addInterests(doc) {
    if (this.interestItems !== undefined && this.interestItems !== [] && this.interestItems.length > 0) {
      doc.setFontSize(14);
      doc.setFontType('bold');
      doc.text('INTERESTS', 20, this.verticalPosition);
      this.verticalPosition += 10;
      doc.setFontSize(12);
      doc.setFontType('normal');
      let interests = '';
  ...
  private addExperiencePageContent(doc) {
    this.addPageTitle(doc, 'PROFESSIONAL EXPERIENCE');
  ...
  private addCvItems(doc, items: CvItem[]) {
    for (const item of items) {
      if (this.verticalPosition >= 235) {
        ...
        doc.text('Continuing on the next page...', 20, this.verticalPosition);
        this.switchPage(doc);
        doc.setTextColor(68, 68, 68);
        doc.setFontType('bolditalic');
        doc.setFontSize(10);
        doc.text('... continued from last page.', 20, this.verticalPosition);
  ...
  private static addPageFooter(doc) {
    ...
    doc.setFontType('normal');
    doc.setFontSize(7);
    doc.text('This CV was generated from a self-hosted online-CV using the AngularCV project.', 20, 284);
    doc.setFontSize(7);
    doc.text('If you received this from a headhunter, ' +
      'he just might have downloaded it from my webpage and sent to you without my consent.', 20, 287);
